            ...
            for joint in range(representation.shape[1]):
                ...
                if representation.shape[1] == 19:
                    if joint == 17:
                        ...
                        p_v = 0
                if mode[joint] == 0:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, delta[joint, 0]:, delta[joint, 1]:], pad_width=((0, 0), (0, delta[joint, 0]), (0, delta[joint, 1])),
                                                      constant_values=p_v)
                elif mode[joint] == 1:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, delta[joint, 0]:, :-delta[joint, 1]], pad_width=((0, 0), (0, delta[joint, 0]), (delta[joint, 1], 0)),
                                                      constant_values=p_v)
                elif mode[joint] == 2:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, :-delta[joint, 0], delta[joint, 1]:], pad_width=((0, 0), (delta[joint, 0], 0), (0, delta[joint, 1])),
                                                      constant_values=p_v)
                elif mode[joint] == 3:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, :-delta[joint, 0], :-delta[joint, 1]],
                                                      pad_width=((0, 0), (delta[joint, 0], 0), (delta[joint, 1], 0)), constant_values=p_v)
                else:
        ...
        if self.move_rep and self.max_motion > 0:
            ...
            for joint in range(representation.shape[1]):
                ...
                if representation.shape[1] == 19:
                    ...
                    p_v = 0
                if mode == 0:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, delta[0, 0]:, delta[0, 1]:], pad_width=((0, 0), (0, delta[0, 0]), (0, delta[0, 1])),
                                                      constant_values=p_v)
                elif mode == 1:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, delta[0, 0]:, :-delta[0, 1]], pad_width=((0, 0), (0, delta[0, 0]), (delta[0, 1], 0)),
                                                      constant_values=p_v)
                elif mode == 2:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, :-delta[0, 0], delta[0, 1]:], pad_width=((0, 0), (delta[0, 0], 0), (0, delta[0, 1])),
                                                      constant_values=p_v)
                elif mode == 3:
                    new_potion_rep[:, joint] = np.pad(representation[:, joint, :-delta[0, 0], :-delta[0, 1]], pad_width=((0, 0), (delta[0, 0], 0), (delta[0, 1], 0)),
                                                      constant_values=p_v)
                else:
                    print('Something wrong')
                    raise NotImplementedError
